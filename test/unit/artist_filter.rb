...
class ArtistFilterTest < ActiveSupport::TestCase
  ...
  test "can get a list of artist's genres" do
    response = ArtistFilter.get_artist_genres(@radiohead_id)
  ...
  test "return true Artist's genres match user requested genres" do
    assert ArtistFilter.is_artist_valid?(@radiohead_id, @user_genre_1)
  end

  test "returns false if artist does not match genre" do
    refute ArtistFilter.is_artist_valid?(@radiohead_id, @user_genre_2)
  end

  test "returns 'couldn't find artist' if artist if unrecognised" do
    assert_equal "Couldn't get artist genres", ArtistFilter.get_artist_genres(@invalid_artist)
  end
