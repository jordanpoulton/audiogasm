...
class ArtistFilterTest < ActiveSupport::TestCase
  ...
  test "return true if any of artist's genres match user requested genres" do
    assert ArtistFilter.should_artist_be_played?(@radiohead_id, @user_genre_array)
  end

  test "return false if none of artist's genres match user requested genre" do
    refute ArtistFilter.should_artist_be_played?(@radiohead_id, @user_genre_array_invalid)
  end
